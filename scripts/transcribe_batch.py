...
def ensure_workspace_layout(paths: PipelinePaths) -> None:
    ...
    ensure_local_config(
        ...
        paths.series_map_path.parent / SERIES_MAP_EXAMPLE_FILENAME,
...
def main() -> int:
    ...
    logger = PipelineTaskLogger(workspace_root=workspace_root, module="transcribe")
    try:
        ...
        print(f"错误：{exc}")
        logger.finish(status="failed", message=str(exc))
        return 2
    except Exception as exc:  # pragma: no cover - 兜底避免裸 traceback
        print(f"错误：{exc}")
        try:
            logger.finish(status="failed", message=str(exc))
        except Exception:
