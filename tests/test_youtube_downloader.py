...
class YouTubeDownloaderTests(unittest.TestCase):
    ...
    def test_sync_source_replaces_old_numbered_audio_when_same_title_downloaded(self) -> None:
        def fake_runner(command: list[str], cwd: Path) -> None:
            del command, cwd
            (audio_dir / "20260101. 新课 A [aaa111].mp3").write_bytes(b"a")
    ...
    def test_sync_source_writes_internal_task_log_events(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            events = [
                json.loads(line)
                for line in (workspace / ".pipeline" / "logs" / "events.jsonl").read_text(encoding="utf-8").splitlines()
            ]
            stages = {(event["stage"], event["status"]) for event in events}
            self.assertIn(("sync_source", "started"), stages)
            self.assertIn(("download_audio", "success"), stages)
