...
from .config import extract_numeric_prefix
...
class AliyunOssSignedUploader(AbstractAudioUploader):
    ...
    def _build_object_filename(self, audio_path: Path) -> str:
        sequence = extract_numeric_prefix(audio_path.name) or 0
        digest = hashlib.sha1(audio_path.name.encode("utf-8")).hexdigest()[:16]
